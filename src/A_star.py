...
class Table:
    ...
    def search(self):
        ...
        for goal in self.p:
            ...
            # start the algorithm
            graph = Graph(Node(self.table, self.r.x, self.r.y, 'N', self.r.heuristicCost, robotGoal))
...
class Node:
    def __init__(self, state, x, y, action, cost, robotGoal):
        self.state = state
        self.r_x = int(x)
        self.r_y = int(y)
        self.action = action
        self.heuristic = 0
        Node.calculateHeuristic(self, robotGoal, cost)

    def calculateHeuristic(self, robotGoal, cost):
        deltaX = abs(robotGoal.x - self.r_x)
        deltaY = abs(robotGoal.y - self.r_y)
        self.heuristic = cost + deltaX + deltaY
